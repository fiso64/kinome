...
import type { LibraryItem, MediaFile, MediaFolder, Person, Settings, TmdbEpisode, TmdbSeason } from '@shared/types'
...
export async function applyMetadataToItem(
    ...
    if (data.genres && Array.isArray(data.genres) && (!respectLocks || !repositoryService.isFieldLocked(item, 'genres'))) {
        item.genres = data.genres.map((g: { name: string }) => g.name)
    }

    // 2. Images
    await downloadAndApplyImage(item, 'poster', data.poster_path, 'https://image.tmdb.org/t/p/w500', `${item.id}.jpg`)
    await downloadAndApplyImage(item, 'backdrop', data.backdrop_path, 'https://image.tmdb.org/t/p/original', `${item.id}-backdrop.jpg`)
    ...
    if (data.images?.logos) {
        ...
        if (bestLogo) {
            ...
            await downloadAndApplyImage(item, 'logo', bestLogo.file_path, 'https://image.tmdb.org/t/p/w500', `${item.id}-logo${extension}`)
        }
    }
}
...
export function applyCreditsToItem(item: LibraryItem, creditsData: any) {
    ...
    const people = new Map<number, { personData: any, actingScore: number, crewScore: number, characters: string[], jobs: string[] }>()
        ...
        if (tmdbSeason) {
            ...
            if (tmdbSeason.poster_path) {
                ...
                if (seasonFolder.posterPath !== posterFileName) {
                    await downloadAndApplyImage(seasonFolder, 'poster', tmdbSeason.poster_path, 'https://image.tmdb.org/t/p/w500', posterFileName, options)
                    seasonChanged = true
                }
            }
        }
    ...
    for (const localEpisode of localEpisodes) {
        ...
        if (tmdbEpisode) {
            ...
            if (tmdbEpisode.still_path) {
                await downloadAndApplyImage(localEpisode, 'poster', tmdbEpisode.still_path, 'https://image.tmdb.org/t/p/w500', `${localEpisode.id}.jpg`)
            }
        } else {
            // Clear metadata if no match found (unless locked)
            if (!options.respectLocks || !repositoryService.isFieldLocked(localEpisode, 'title')) localEpisode.title = undefined
            if (!options.respectLocks || !repositoryService.isFieldLocked(localEpisode, 'overview')) localEpisode.overview = undefined
            if (!options.respectLocks || !repositoryService.isFieldLocked(localEpisode, 'posterPath')) localEpisode.posterPath = undefined
        }
