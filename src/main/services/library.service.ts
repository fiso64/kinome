...
export async function refreshLibrary(): Promise<MediaFolder | null> {
    if (pathsService.isRemoteLibrary()) throw new Error('Refreshing not available for remote libraries.')
...
async function _scanAndCreateNewDb(
    ...
    if (pathsService.isRemoteLibrary()) throw new Error(`${scanType} not available for remote libraries.`)
...
export async function getItemDetails(itemId: string): Promise<LibraryItem | null> {
    ...
    metadataService.backgroundFetchAndApplyDetails(item)
...
export async function markAsUnwatched(itemId: string): Promise<void> {
    ...
    repositoryService.runTransaction(() => {
        for (const i of itemsToUpdate) {
            ...
            if (i.type === 'folder') {
                repositoryService.updateItem(i.id, { continueWatchingDismissed: false, nextUpDismissed: false })
            }
        }
    })
...
export async function getContinueWatchingItems(includeDismissed = false): Promise<{ show: MediaFolder; nextEpisode: MediaFile }[]> {
    const allItems = repositoryService.getAllItemsAsList()
    ...
    const adjacency = db.prepare('SELECT id, parent_id FROM items').all() as { id: string, parent_id: string }[]
    ...
    for (const show of shows) {
        ...
        for (const item of allItems) {
            if (item.type === 'file' && item.mediaType === 'episode') {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    if (!pid) break
                    if (pid === show.id) { isChild = true; break; }
                    currentId = pid
                }
        ...
        for (let i = 0; i < episodes.length; i++) {
            if (episodes[i].watched) {
                if (maxWatchedIdx === -1 || getComparable(episodes[i]) > getComparable(episodes[maxWatchedIdx])) {
                    maxWatchedIdx = i
                }
            }
        }
        ...
        for (let i = 0; i < episodes.length; i++) {
            if (!episodes[i].watched && getComparable(episodes[i]) > getComparable(episodes[maxWatchedIdx])) {
        ...
        if (nextEpisode) {
            if (!nextEpisode.title) {
                const updatedItems = await metadataService.fetchEpisodeDataForContinueWatching(show, nextEpisode)
...
export const assignSeasonsAndEpisodes = async (showId: string, s1: any, s2: any, fm: boolean) => {
    ...
    if (fm) {
        await retrieverService.refetchShowSeasons(show, await settingsService.readSettings(), pathsService.getLibraryDataPath())
    }
...
export const applyTmdbResult = async (itemId: string, result: any, mediaType: 'movie' | 'tv' | 'season') => {
...
export const setImage = async (itemId: string, imageType: 'poster' | 'backdrop' | 'logo', source: { type: 'tmdb'; path: string } | { type: 'local'; path: string }) => {
...
export const fetchCredits = async (itemId: string) => {
    const item = repositoryService.getItemById(itemId)
    if (item) {
        await retrieverService.fetchAndApplyCredits(item, (await settingsService.readSettings()).tmdbApiKey)
        repositoryService.updateItem(item.id, { tmdbCredits: item.tmdbCredits, tmdbCreditsFetched: true })
...
// Missing Exports
export const applyInitialFolderSettings = async (settings: { id: string; retrieve: boolean; hint?: 'movie' | 'tv' }[]) => {
    repositoryService.runTransaction(() => {
...
export const getFolderWatchedState = async (folderId: string): Promise<'fully' | 'partially' | 'unwatched' | 'none'> => {
