...
import type { Database, MediaFolder, LibraryItem, MediaFile, AutocompleteSuggestions, Settings } from '../../shared/types'
...
async function readDb(): Promise<Database | null> {
  try {
    ...
    if (parsedDb.version !== DB_VERSION) {
      console.warn(`Database version mismatch. Expected ${DB_VERSION}, got ${parsedDb.version}. Ignoring old DB.`)
      return null
    }
    return parsedDb
  } catch (e) {
...
async function getDirectoryContentStats(dirPath: string): Promise<{ totalSize: number; fileCount: number; folderCount: number }> {
...
function parseEpisodeInfo(name: string): { season?: number; episode: number; pattern: 'sxxexx' | 'episode_xx' | 'exx' } | null {
...
function processAndAssignEpisodeNumbers(files: MediaFile[], parentSeasonNumber?: number): boolean {
    ...
    for (const currentPattern of patterns) {
        ...
        if (mismatches === 0 || (mismatches <= 2 && matches.length >= 3)) {
            log(`TV Structure: Applying pattern "${currentPattern}". Matches: ${matches.length}, Mismatches: ${mismatches}`)
...
function processTvShowStructure(showFolder: MediaFolder): void {
  ...
  const allSubFolders = showFolder.children.filter((c) => c.type === 'folder' && !SPECIAL_FOLDER_NAMES_FOR_TV.includes(c.name.toLowerCase())) as MediaFolder[]
...
function assignEpisodesByStrategy(files: MediaFile[], seasonNumber: number, strategy: 'smart' | 'alphabetic') {
    ...
    if (!parsedSuccessfully) {
        log('TV Structure (Smart Fallback): High-confidence parsing failed, falling back to alphabetical.')
        files.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }))
        files.forEach((file, index) => {
            file.seasonNumber = seasonNumber
            file.episodeNumber = index + 1
...
async function clearTvStructureMetadata(folder: MediaFolder, imagesDir: string, modifiedItems: Set<LibraryItem>): Promise<void> {
...
async function processInChunks<T>(items: T[], concurrencyLimit: number, task: (item: T) => Promise<void>): Promise<void> {
    const queue = [...items]
...
function collectItemsToProcess(folder: MediaFolder, newItems: { item: LibraryItem; hint?: 'movie' | 'tv' }[], itemsMissingPosters: LibraryItem[], itemsMissingCredits: LibraryItem[], tvShows: MediaFolder[]) {
    ...
    if (folder.retrieve_children_metadata) {
        for (const child of folder.children) {
            ...
            if (typeof child.tmdbId === 'undefined') {
                ...
                if (!child.tmdbCreditsFetched && (child.mediaType === 'movie' || child.mediaType === 'tv')) {
...
async function fetchMetadataForLibrary() {
    ...
    collectItemsToProcess(db.root, newItemsToFetch, itemsMissingPosters, itemsMissingCredits, allTvShows)
        ...
        for (const show of allTvShows) {
            ...
            if (show.tmdbSeasons && show.tmdbDetailsFetched) {
                ...
                if (localSeasonNumbers.size > 0) {
                    ...
                    const hasUnprocessedSeason = show.children.some((c) => c.type === 'folder' && c.mediaType === 'season' && c.seasonNumber != null && !c.title)
                    if (needsRefetch || hasUnprocessedSeason) {
                        log(`[Metadata] New or unprocessed seasons detected for "${show.name}". Fetching updated season list.`)
                        await refetchShowSeasons(show, settings, libraryDataPath)
                    }
                }
            }
            const seasonFolders = show.children.filter((c) => c.type === 'folder' && c.mediaType === 'season') as MediaFolder[]
            for (const season of seasonFolders) {
                if (season.tmdbEpisodes && season.tmdbEpisodesFetched) {
                    ...
                    if (maxLocalEpisode > maxCachedEpisode && maxLocalEpisode > (season._lastSeenLocalMaxEpisode ?? 0)) {
                        log(`[Metadata] New episode for "${show.name} S${season.seasonNumber}" (new local max: ${maxLocalEpisode}, last: ${season._lastSeenLocalMaxEpisode}). Invalidating episode cache.`)
    ...
    if (newItemsToFetch.length === 0 && itemsMissingPosters.length === 0 && itemsMissingCredits.length === 0) {
    ...
    if (newItemsToFetch.length > 0) {
        ...
        const task = async (itemWithHint: { item: LibraryItem; hint?: 'movie' | 'tv' }): Promise<void> => {
            const { item, hint } = itemWithHint
            await searchTmdbAndApplyMetadata(item, settings.tmdbApiKey, libraryDataPath, hint)
            if (item.type === 'folder' && item.mediaType === 'tv') processTvShowStructure(item as MediaFolder)
    ...
    try {
        ...
        if (seasonFolder) {
            if (!seasonFolder.tmdbEpisodesFetched) {
                log(`[Continue Watching] Next episode "${episode.name}" is in an unfetched season. Fetching S${seasonFolder.seasonNumber}...`)
                const modifiedEpisodes = await fetchAndApplyEpisodeData(seasonFolder, show.tmdbId, settings.tmdbApiKey, getLibraryDataPath(), show.tmdbSeasons)
                itemsToUpdate = [seasonFolder, ...modifiedEpisodes]
            }
        } else {
            if (!show.tmdbEpisodesFetched) {
                log(`[Continue Watching] Next episode "${episode.name}" is a loose file in an unfetched show. Fetching all loose episodes...`)
...
async function resetItemMetadata(item: LibraryItem, imagesDir: string) {
  if (!isRemoteLibrary()) {
    if (item.posterPath) try { await fs.unlink(path.join(imagesDir, item.posterPath)) } catch (e) {}
    if (item.backdropPath) try { await fs.unlink(path.join(imagesDir, item.backdropPath)) } catch (e) {}
    if (item.logoPath) try { await fs.unlink(path.join(imagesDir, item.logoPath)) } catch (e) {}
  }
  for (const key of RESETTABLE_METADATA_KEYS) {
    if (key in item) {
      const itemAsAny = item as any
      switch (key) {
        case 'tags': itemAsAny[key] = {}; break
        case 'genres': itemAsAny[key] = []; break
        case 'tmdbDetailsFetched': case 'tmdbEpisodesFetched': case 'tmdbCreditsFetched': itemAsAny[key] = false; break
        case 'virtualTags': itemAsAny[key] = undefined; break
        case 'posterPath': case 'backdropPath': case 'logoPath': itemAsAny[key] = undefined; break
        default: itemAsAny[key] = null; break
      }
    }
  }
  item._v = Date.now()
...
async function clearChildrenRecursively(folder: MediaFolder, imagesDir: string, modifiedItems: LibraryItem[]): Promise<void> {
    for (const child of folder.children) {
...
export async function refreshLibrary(): Promise<MediaFolder | null> {
    ...
    try {
        ...
        throw { message: 'The configured media source path could not be found.', detail: `Path: ${mediaSourcePath}` }
    }
    setBulkUpdateStatus(true)
    await syncWithDisk(db.root, mediaSourcePath)
    const t1 = performance.now(); log(`[Refresh ${refreshId}] syncWithDisk took ${(t1 - t0).toFixed(2)}ms`)
    pruneUntouchedMissingItems(db.root)
    const t2 = performance.now(); log(`[Refresh ${refreshId}] pruneUntouchedMissingItems took ${(t2 - t1).toFixed(2)}ms`)
    const imagesDir = path.join(getLibraryDataPath(), 'images')
    await verifyImagePaths(db.root, imagesDir)
    const t3 = performance.now(); log(`[Refresh ${refreshId}] verifyImagePaths took ${(t3 - t2).toFixed(2)}ms`)
    const currentSettings = await readSettings()
    applyVirtualTagsToAllItems(db.root, currentSettings)
    const t4 = performance.now(); log(`[Refresh ${refreshId}] applyVirtualTagsToAllItems took ${(t4 - t3).toFixed(2)}ms`)
    buildFullSearchIndex(db.root)
    const t5 = performance.now(); log(`[Refresh ${refreshId}] buildFullSearchIndex took ${(t5 - t4).toFixed(2)}ms`)
    setBulkUpdateStatus(false)
    await writeDb(db)
    const t6 = performance.now(); log(`[Refresh ${refreshId}] writeDb took ${(t6 - t5).toFixed(2)}ms`)
    log(`[Refresh ${refreshId}] Library refresh and sync complete.`)
    fetchMetadataForLibrary().catch((err) => console.error('Background metadata fetch failed during refresh:', err))
...
export async function getItemDetails(itemId: string): Promise<LibraryItem | null> {
    ...
    const needsEpisodeFetch = item.type === 'folder' && (item.mediaType === 'tv' || item.mediaType === 'season') && !item.tmdbEpisodesFetched
                    ...
                    if (item.mediaType === 'season') {
                        ...
                        if (showFolder && showFolder.tmdbId && showFolder.process_tv_children !== false) {
                            if (!showFolder.tmdbDetailsFetched) {
                                ...
                                const modifiedParent = await fetchItemDetails(showFolder, settings, getLibraryDataPath())
                                allModifiedItems.push(...modifiedParent)
                            }
                            if (showFolder.tmdbSeasons) {
                                log(`[Details] Season episodes missing. Fetching for "${item.name}"`)
                                const modifiedEpisodes = await fetchAndApplyEpisodeData(item, showFolder.tmdbId, settings.tmdbApiKey, getLibraryDataPath(), showFolder.tmdbSeasons)
                                allModifiedItems.push(item, ...modifiedEpisodes)
                            } else {
                                item.tmdbEpisodesFetched = true
                                log(`[Details] Could not fetch episodes for season "${item.name}" (parent has no season data). Marked as processed to prevent loops.`)
                            }
                        } else {
                            item.tmdbEpisodesFetched = true
                            log(`[Details] Could not fetch episodes for season "${item.name}" (preconditions not met). Marked as processed to prevent loops.`)
                        }
...
export async function fetchCredits(itemId: string): Promise<void> {
    ...
    const needsCreditsFetch = !item.tmdbCreditsFetched && item.tmdbId && (item.mediaType === 'movie' || item.mediaType === 'tv')
...
export async function playFileWith(file: MediaFile, command: string, onError: ErrorCallback): Promise<boolean> {
    if (!command) return false
    const mediaSourcePath = await getAbsoluteMediaSourcePath()
    if (!mediaSourcePath) {
        onError({ title: 'Configuration Error', message: 'Media source path is not configured. Please check your library settings.' })
        return false
    }
    ...
    exec(commandToExecute, (error) => {
        if (error) {
            onError({ title: 'Player Error', message: 'Failed to launch the external player.', detail: `Command: ${commandToExecute}\nError: ${error.message}` })
        }
    })
    ;(async () => {
        if (!db?.root) return
...
export async function playFile(file: MediaFile, onError: ErrorCallback): Promise<boolean> {
    ...
    if (!playerCommands || playerCommands.length === 0 || !playerCommands[0].command) {
        onError({ title: 'Configuration Error', message: 'Player command is not configured. Please set it in Settings.' })
        return false
    }
...
export async function applyInitialFolderSettings(settings: { id: string; retrieve: boolean; hint?: 'movie' | 'tv' }[]) {
    ...
    try {
        ...
        fetchMetadataForLibrary().catch((err) => console.error('Background metadata fetch failed after applying initial settings:', err))
    } catch (error) {
...
export async function getAutocompleteSuggestions(): Promise<AutocompleteSuggestions> {
  if (!db || !db.root) return { mediaTypes: [], genres: [], persons: [], tagKeys: [], virtualTagKeys: [], tagValues: {} }
  const allItems = getAllItemsAsList(db.root)
      ...
      for (const [key, value] of Object.entries(item.tags)) {
        if (key) {
          ...
          value.split(',').forEach((v) => {
            const trimmedV = v.trim(); if (trimmedV) tagValues[key].add(trimmedV)
          })
        }
      }
    }
...
export async function executeCustomAction(itemId: string, commandId: string, onError: ErrorCallback): Promise<void> {
    if (!db?.root) return
    const item = findItemById(itemId, db.root)
    if (!item) return
    const settings = await readSettings()
    ...
    exec(commandToExecute, (error) => {
        if (error) {
            onError({ title: 'Custom Action Error', message: 'Failed to execute the custom command.', detail: `Command: ${commandToExecute}\nError: ${error.message}` })
        }
    })
}

export async function applyTmdbResult(itemId: string, result: any, mediaType: 'movie' | 'tv' | 'season', onError: ErrorCallback) {
    if (isRemoteLibrary()) {
        onError({ title: 'Operation Not Supported', message: 'Applying metadata is not available for read-only remote libraries.' })
        return
    }
    if (!db?.root) return
    ...
    setBulkUpdateStatus(true)
    try {
        item.overview = null; item.year = null; item.genres = []
        if (item.type === 'file') item.opensAsFolder = true
        item.posterPath = undefined; item.backdropPath = undefined; item.logoPath = undefined
        item.tmdbDetailsFetched = false; item.tmdbCreditsFetched = false; item.tmdbCredits = null
        if (item.type === 'folder' && mediaType === 'tv') {
            item.tmdbSeasons = null; item.tmdbEpisodesFetched = false
            for (const season of item.children) {
                if (season.type === 'folder' && season.mediaType === 'season') {
                    season.tmdbDetailsFetched = false; season.tmdbEpisodesFetched = undefined
                    for (const episode of season.children) {
                        if (episode.type === 'file' && episode.mediaType === 'episode') episode.posterPath = undefined
                    }
                }
            }
        }
        if (mediaType === 'season' && item.type === 'folder') {
            item.mediaType = 'season'; item.title = result.name; item.overview = result.overview; item.seasonNumber = result.season_number
            if (result.poster_path) {
                const posterUrl = `https://image.tmdb.org/t/p/w500${result.poster_path}`
                const posterDestPath = path.join(getLibraryDataPath(), 'images', `${item.id}.jpg`)
                try {
                    await downloadImage(posterUrl, posterDestPath); item.posterPath = `${item.id}.jpg`
                } catch (e) { console.error('Failed to download season poster', e) }
            }
            item.tmdbDetailsFetched = true; item.tmdbEpisodesFetched = undefined
            ...
            if (!episodeFiles.some((ef) => typeof ef.episodeNumber !== 'undefined')) {
                if (!processAndAssignEpisodeNumbers(episodeFiles, item.seasonNumber)) {
                    ...
                    episodeFiles.forEach((file, index) => {
                        file.episodeNumber = index + 1; file.seasonNumber = item.seasonNumber; file.mediaType = 'episode'
                    })
                }
            }
            const showFolder = findParent(item.id, db!.root!)
            if (showFolder?.tmdbId && settings.tmdbApiKey) {
                if (!showFolder.tmdbDetailsFetched) await fetchItemDetails(showFolder, settings, libraryDataPath)
                await fetchAndApplyEpisodeData(item, showFolder.tmdbId, settings.tmdbApiKey, libraryDataPath, showFolder.tmdbSeasons)
            }
        } else {
            item.tmdbId = result.id; item.mediaType = mediaType; item.title = result.title
            ...
            if (mediaType === 'movie' || mediaType === 'tv') await fetchAndApplyCredits(item, settings.tmdbApiKey)
        }
    } finally {
        setBulkUpdateStatus(false)
    }
...
export async function markAsUnwatched(itemId: string): Promise<void> {
    ...
    function setUnwatchedRecursively(node: LibraryItem) {
        if (node.type === 'file' && node.watched) {
            node.watched = false; node.lastWatched = undefined; modifiedItems.push(node)
        }
        if (node.type === 'folder') {
            let wasModified = false
            if (node.continueWatchingDismissed) { node.continueWatchingDismissed = false; wasModified = true }
            if (node.nextUpDismissed) { node.nextUpDismissed = false; wasModified = true }
...
export async function markAsWatched(itemId: string): Promise<void> {
    ...
    function setWatchedRecursively(node: LibraryItem) {
        if (node.type === 'file' && !node.watched) {
            node.watched = true; (node as MediaFile).lastWatched = Date.now(); modifiedItems.push(node)
        }
...
export async function assignSeasonsAndEpisodes(showId: string, seasonStrategy: 'smart' | 'alphabetic', episodeStrategy: 'smart' | 'alphabetic', fetchMetadata: boolean) {
    ...
    try {
        ...
        log(`[Assign Seasons] Re-applying virtual tags and re-indexing for ${modifiedItems.size} items...`)
        for (const item of modifiedItems) {
            item.virtualTags = evaluateVirtualTagsForItem(item, settings)
            updateIndexForItem(item)
        }
...
export async function setImage(itemId: string, imageType: 'poster' | 'backdrop' | 'logo', source: { type: 'tmdb'; path: string } | { type: 'local'; path: string }, onError: ErrorCallback) {
    if (isRemoteLibrary()) {
        onError({ title: 'Operation Not Supported', message: 'Setting images is not available for remote libraries.' })
        return
    }
    if (!db?.root) return
    markAsUserEdited(itemId, db.root)
    ...
    let fileName = ''
    switch (imageType) {
        case 'poster': fileName = `${item.id}${extension || '.jpg'}`; break
        case 'backdrop': fileName = `${item.id}-backdrop${extension || '.jpg'}`; break
    ...
    try {
        ...
        onError({ title: 'Image Error', message: 'Failed to set the selected image. See logs for more details.' })
    }
}
...
export async function getAbsolutePath(relativePath: string): Promise<string | null> {
    ...
    if (isRemotePath(mediaSourcePath)) {
        return new URL(relativePath, mediaSourcePath + (mediaSourcePath.endsWith('/') ? '' : '/')).toString()
    }
...
export async function handleItemRenamed(relativeOldPath: string, newName: string): Promise<void> {
    ...
    function updatePathsAndIds(item: LibraryItem, newParentPath: string) {
        ...
        item.virtualTags = evaluateVirtualTagsForItem(item, settings)
        if (item.type === 'folder') item.children.forEach((child) => updatePathsAndIds(child, item.path))
        updateIndexForItem(item)
    }
...
export async function getItemProperties(relativePath: string): Promise<any | null> {
    ...
    try {
        ...
        const baseProperties = { name: path.basename(absolutePath), path: absolutePath, type: stats.isDirectory() ? 'Folder' as const : 'File' as const, created: stats.birthtime.toISOString(), modified: stats.mtime.toISOString() }
        if (stats.isDirectory()) {
            const contentStats = await getDirectoryContentStats(absolutePath)
            return { ...baseProperties, size: contentStats.totalSize, contains: { files: contentStats.fileCount, folders: contentStats.folderCount } }
        } else {
...
export async function getContinueWatchingItems(): Promise<{ show: MediaFolder; nextEpisode: MediaFile }[]> {
    if (!db?.root) return []
    const allItems = getAllItemsAsList(db.root)
    ...
    for (const item of allItems) {
        if (item.type === 'file' && item.mediaType === 'episode') {
            let currentParentId = parentMap.get(item.id), showId: string | null = null
            while (currentParentId) {
                const parent = findItemById(currentParentId, db.root)
                if (parent?.type === 'folder' && parent.mediaType === 'tv') { showId = parent.id; break }
    ...
    const getComparableEpisodeNumber = (ep: MediaFile) => (ep.seasonNumber ?? 0) * 10000 + (ep.episodeNumber ?? 0)
    const continueWatchingItems: { show: MediaFolder; nextEpisode: MediaFile; lastWatched: number }[] = []
    for (const [showId, show] of shows.entries()) {
        ...
        const allEpisodesSorted = [...episodes].sort((a, b) => getComparableEpisodeNumber(a) - getComparableEpisodeNumber(b))
        const maxWatchedEpisode = watchedEpisodes.reduce((max, ep) => (getComparableEpisodeNumber(ep) > getComparableEpisodeNumber(max) ? ep : max))
        ...
        for (const episode of allEpisodesSorted) {
            if (getComparableEpisodeNumber(episode) > getComparableEpisodeNumber(maxWatchedEpisode)) {
                if (!episode.watched) { nextUnwatchedEpisode = episode; break }
            }
        }
    ...
    return continueWatchingItems.sort((a, b) => b.lastWatched - a.lastWatched).map(({ show, nextEpisode }) => ({ show, nextEpisode }))
}

export async function getContinueWatchingForShow(showId: string): Promise<{ show: MediaFolder; nextEpisode: MediaFile } | null> {
    if (!db?.root) return null
    const show = findItemById(showId, db.root)
    if (show?.type !== 'folder' || show.mediaType !== 'tv' || show.nextUpDismissed) return null
    const episodes = (getAllItemsAsList(show) as LibraryItem[]).filter((item) => item.type === 'file' && item.mediaType === 'episode') as MediaFile[]
