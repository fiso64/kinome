...
import { serviceEventEmitter } from '../services/event.emitter.service'
...
export function setupIpcHandlers() {
  ...
  // Listen for events from services and forward them to the renderer process.
  // This decouples services from the `electron` module.
  serviceEventEmitter.on('library-items-updated', (items: LibraryItem[]) => {
    BrowserWindow.getAllWindows().forEach((window) => {
      window.webContents.send('library-items-updated', items)
    })
  })

  serviceEventEmitter.on('library-item-updated', (item: LibraryItem) => {
    BrowserWindow.getAllWindows().forEach((window) => {
      window.webContents.send('library-item-updated', item)
    })
  })

  serviceEventEmitter.on('autocomplete-suggestions-updated', (suggestions: AutocompleteSuggestions) => {
    BrowserWindow.getAllWindows().forEach((window) => {
      window.webContents.send('autocomplete-suggestions-updated', suggestions)
    })
  })

  serviceEventEmitter.on('library-item-deleted', (itemId: string) => {
    BrowserWindow.getAllWindows().forEach((window) => {
  ...
  ipcMain.handle('get-autocomplete-suggestions', () => libraryService.getAutocompleteSuggestions())
