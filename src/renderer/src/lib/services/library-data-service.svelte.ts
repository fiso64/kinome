...
import { QueryThrottler } from '../query-throttler'
import { api } from '../api'
...
class LibraryDataService {
    ...
    private readonly keys = {
        ...
        children: {
            ...
            byParent: (parentId: string | null | undefined, fields: string[] = [], groupBy?: string) =>
                [...this.keys.children.all, parentId, { fields, groupBy }] as const,
        },
        continueWatching: {
            all: ['continue-watching'] as const,
            forShow: (showId: string | null | undefined) => [...this.keys.continueWatching.all, showId] as const,
        },
        parent: {
    ...
    getChildrenQuery(
        ...
        return createQuery(() => {
            ...
            return {
                queryKey: this.keys.children.byParent(normalizedId, fields, groupBy),
                queryFn: () =>
                    normalizedId
                        ? api.getChildrenV2(parentId!, {
                            include: fields,
                            groupBy: groupBy
                        })
                        : [],
     ...
     */
    getContinueWatchingForShowQuery(showIdFn: () => string | null | undefined, options: { enabled?: () => boolean } = {}) {
        return createQuery(() => {
     ...
     */
    getParentQuery(itemIdFn: () => string | null | undefined, options: { enabled?: () => boolean } = {}) {
        return createQuery(() => {
    ...
    handleLibraryUpdates(updatedItems: LibraryItem[], isScanning: boolean) {
        ...
        for (const item of updatedItems) {
            ...
            this.queryClient.setQueriesData(
                { queryKey: this.keys.children.all },
                (oldData: any) => {
            ...
            const isCWRelated =
                ...
                (item.type === 'folder' && ('continueWatchingDismissed' in item || 'nextUpDismissed' in item))
