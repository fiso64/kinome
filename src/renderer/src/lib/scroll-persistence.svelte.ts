...
export function scrollPersistence(node: HTMLElement, options: ScrollPersistenceOptions) {
  ...
  if (!options.disabled && options.resetOn !== undefined) {
    ...
    console.log(
      `[ScrollPersistence:${options.key}] INIT CHECK | Store: '${store.resetVal}' | Current: '${currentVal}'`
    )
  ...
  return {
    update(newOptions: ScrollPersistenceOptions) {
      ...
      if (!newOptions.disabled && newOptions.resetOn !== undefined) {
        ...
        const store = viewStateStore.get(newOptions.key, {
          y: 0,
          x: 0,
          resetVal: null as string | null
        })

        if (store.resetVal !== currentVal) {
          console.log(
            `[ScrollPersistence:${newOptions.key}] RESET (UPDATE) | Store: '${store.resetVal}' => Current: '${currentVal}'`
          )
          store.y = 0
          store.x = 0
          store.resetVal = currentVal
