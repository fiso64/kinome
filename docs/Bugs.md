...
1. It is not performant, because it sends huge amounts of data, even when not everything is needed (such as sending a huge library root object that contains even the cast, seemingly the entire db!). It should only send and request exactly what is needed, when needed.
2. It is not maintainable or scalable, because we haven't clearly defined what triggers what, and when. This leads to unpredictable results such as the above. It is not clearly defined. Adding a new feature always leads to confusion: How much should the frontend be responsible vs the backend? We must discuss this and rigorously define the API, maybe write a spec.
