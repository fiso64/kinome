...
// --- Configuration ---
const PORT = 3002 // Use different port than basic test
...
async function setupFileSystem() {
    try {
        ...
        await fs.writeFile(path.join(LIBRARY_PATH, 'TV', 'Breaking Bad', 'Season 1', 'Breaking Bad S01E01.mp4'), 'dummy')
        await fs.writeFile(path.join(LIBRARY_PATH, 'TV', 'Breaking Bad', 'Season 1', 'Breaking Bad S01E02.mp4'), 'dummy')

        // 3. TV Show Setup (Alphabetic Fallback)
        await fs.mkdir(path.join(LIBRARY_PATH, 'TV', 'Unknown Show', 'Season 1'), { recursive: true })
        await fs.writeFile(path.join(LIBRARY_PATH, 'TV', 'Unknown Show', 'Season 1', 'Chapter 1.mp4'), 'dummy')
        await fs.writeFile(path.join(LIBRARY_PATH, 'TV', 'Unknown Show', 'Season 1', 'Chapter 2.mp4'), 'dummy')
